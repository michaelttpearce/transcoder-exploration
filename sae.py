...
class UberSAE(BaseTopKSAE):
    ...
    def __init__(self, cfg, model_cfg):
        super().__init__()
        # model_cfg: dict of {'model': HookedTransformer, 'sae': sae, 'meta_sae': MetaSAE, 'hook_point': hook_point, 'layer': layer} to avoid parameters being added
        self.cfg = cfg
    ...
    def get_dataset(self):
        ...
        tokenized_dataset = tokenize_and_concatenate(dataset, self.model_cfg['model'].tokenizer, 
    ...
    def get_dataloader(self, batch_size=None):
        ...
        return iter(DataLoader(dataset, batch_size=batch_size))import torch
import torch.nn as nn
from dataclasses import dataclass
from datasets import load_dataset
from tqdm import tqdm
...
class BaseSAE(torch.nn.Module):
    ...
    def step(self, batch):
        x = self.get_inputs(batch)
        x_hat, acts, x_hat_aux = self.forward(x)
        ...
        metrics = {}
        mse_loss = (x - x_hat)**2
        aux_mse_loss = (x - x_hat_aux)**2 if x_hat_aux is not None else 0
        ...
        metrics['loss'] = self.cfg.mse_param * mse_loss.mean() + self.cfg.aux_mse_param * aux_mse_loss.mean() + self.cfg.feature_sim_param * feature_sim_loss
...
class BaseTopKSAE(BaseSAE):
    ...
    def forward(self, x):
        ...
        if self.cfg.aux_mse_param > 0:
            x_hat_aux = self.decoder(pre_acts) + self.b_pre
        else:
            x_hat_aux = None
        return x_hat, acts, x_hat_aux
